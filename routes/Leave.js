...
router.post('/', async (req, res) => {
  try {
    ...
    if (existingLeave) {
      return res
        .status(400)
        .json({ message: 'A leave request already exists for these dates', existingLeave });
    }
    ...
    if (type === 'Sick' && sickLeaveRemaining < leaveDays) {
      return res
        .status(400)
        .json({ message: `Not enough Sick Leaves. Available: ${sickLeaveRemaining}` });
    }
    if (type === 'Annual' && annualLeaveRemaining < leaveDays) {
      return res
        .status(400)
        .json({ message: `Not enough Annual Leaves. Available: ${annualLeaveRemaining}` });
    }
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    let lastLeave = await Leave.findOne({ userId: leave.userId, year: leave.year }).sort({
      createdAt: -1,
    });
