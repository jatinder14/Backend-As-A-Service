...
router.post('/:leadId', async (req, res) => {
  ...
  try {
    ...
    if (lead?.createdAt != req.user.id) {
      return res
        .status(403)
        .json({ message: 'Only the sales manager of the lead can upload the documents.' });
    }
...
router.put('/:leadId', async (req, res) => {
  ...
  try {
    ...
    if (lead?.createdAt != req.user.id) {
      return res
        .status(403)
        .json({ message: 'Only the sales manager of the lead can update the documents.' });
    }
    ...
    if (bankAccount.endDate < bankAccount.startDate) {
      return res
        .status(400)
        .json({ error: 'End date must be greater than or equal to start date' });
    }
