...
router.get('/:id', async (req, res) => {
  try {
    const booking = await Booking.findById(req.params.id).populate('customerId propertyId');
    if (!booking) {
...
router.post('/', async (req, res) => {
  const { customerId, propertyId, bookingDates, paymentStatus, confirmationDetails, totalPrice } =
    req.body;
  ...
  try {
    ...
    if (duration < minDuration) {
      return res.status(400).json({ message: `Minimum stay is ${minDuration} day(s).` });
    }
    ...
    if (overlappingBooking) {
      return res.status(400).json({
        message: 'Property is already booked during the requested dates',
      });
    }
...
router.put('/:id', async (req, res) => {
  const { bookingDates, paymentStatus, confirmationDetails, totalPrice } = req.body;
  ...
  try {
    ...
    booking.confirmationDetails = confirmationDetails || booking.confirmationDetails;
