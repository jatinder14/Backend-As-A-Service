...
router.get('/', async (req, res) => {
    try {
        const { page = 1, limit = 10, status, isCustom, sortBy = 'sortOrder', order = 'asc' } = req.query;
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const allowedUpdates = [
            'name', 'slug', 'description', 'shortDescription', 'price', 'currency',
            'billingCycles', 'features', 'limits', 'addons', 'status', 'sortOrder',
            'trialDays', 'setupFee', 'cancellationPolicy', 'customCancellationDays',
            'metadata'
        ];
...
router.post('/compare', async (req, res) => {
    try {
        ...
        if (plans.length < 2) {
            return res.status(400).json({ message: 'At least 2 active plans are required for comparison' });
        }
