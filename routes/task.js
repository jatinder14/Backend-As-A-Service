    ...
    try {
        if (listingId) {
            ...
            if (!listing) {
                return res
                    .status(404)
                    .json({ message: ERROR_MESSAGES.LISTING_NOT_FOUND });
            }
        }
        ...
        res.status(500).json({
            message: ERROR_MESSAGES.ERROR_CREATING_TASK,
            error: err.message,
        });
    }
});
...
router.get('/:id', async (req, res) => {
    try {
        ...
        res.status(500).json({
            message: ERROR_MESSAGES.ERROR_FETCHING_TASK,
            error: err.message,
        });
    }
});
...
router.put('/:id', async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({
            message: ERROR_MESSAGES.ERROR_UPDATING_TASK,
            error: err.message,
        });
    }
});
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        if (!task.createdBy.equals(req.user.id)) {
            return res
                .status(403)
                .json({ message: ERROR_MESSAGES.UNAUTHORIZED_DELETE });
        }
        ...
        if (creator) {
            ...
            emailPromises.push(
                sendEmail(creator.email, creatorSubject, creatorText)
            );
        }
        ...
        res.status(500).json({
            message: ERROR_MESSAGES.ERROR_DELETING_TASK,
            error: err.message,
        });
    }
});
