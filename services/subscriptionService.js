...
class SubscriptionService {
    ...
    static async changeSubscription(subscriptionId, newPlanId, options = {}) {
        try {
            ...
            const orderType = newDiscountedPrice > subscription.planSnapshot.price ? 'upgrade' : 'downgrade';
    ...
    static async renewSubscription(subscriptionId) {
        try {
            ...
            const newNextBillingDate = this.calculateNextBillingDate(newStartDate, subscription.billingCycle);
    ...
    static async checkUserAccess(userId, resourceType, resourceCount = 1) {
        try {
            ...
            switch (resourceType) {
                case 'properties':
                    if (limits.properties !== -1 && user.usageStats.propertiesCreated + resourceCount > limits.properties) {
                        hasAccess = false;
                ...
                case 'users':
                    ...
                    break;
                case 'storage':
                    if (limits.storage !== -1 && user.usageStats.storageUsed + resourceCount > limits.storage) {
                        hasAccess = false;
                        reason = 'Storage limit exceeded';
                    }
                    break;
                case 'apiCalls':
                    if (limits.apiCalls !== -1 && user.usageStats.apiCallsUsed + resourceCount > limits.apiCalls) {
                        hasAccess = false;
