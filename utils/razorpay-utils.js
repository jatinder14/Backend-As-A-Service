...
function getTestError(summary, expectedVal, gotVal) {
  ...
  return new Error(
    '\n' +
      ...
      '\n' +
      ('Expected(' +
        (typeof expectedVal === 'undefined' ? 'undefined' : _typeof(expectedVal)) +
        ')\n' +
        prettify(expectedVal) +
        '\n\n') +
      ('Got(' +
        (typeof gotVal === 'undefined' ? 'undefined' : _typeof(gotVal)) +
        ')\n' +
        prettify(gotVal))
  );
}
...
function validatePaymentVerification() {
  ...
  if (isDefined(params.order_id) === true) {
    ...
    var payload =
      paymentLinkId + '|' + paymentLinkRefId + '|' + paymentLinkStatus + '|' + paymentId;
  } else {
