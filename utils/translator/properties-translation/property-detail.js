...
const getSoldStatusLabel = data => {
  ...
  } else if (data?.soldOut && (data?.status === 'OFF_PLAN' || data?.status === 'SALE')) {
    return 'Sold';
...
const parseTranslatedArray = (input, language) => {
  try {
    const decoded = decodeHtml(typeof input === 'string' ? input : JSON.stringify(input));
    ...
    const fallbackDecoded = decodeHtml(typeof input === 'string' ? input : JSON.stringify(input));
    const delimiter = getLanguageDelimiter(language);
    return fallbackDecoded.split(delimiter).map(stripOuterQuotes);
  }
};

export const translatePropertyDetails = async (dynamicText, globalState) => {
  let updatedObject = {};
  ...
  try {
    ...
    // let features = [];
    const features = parseTranslatedArray(convertedArray[8], globalState?.selectedLanguage);
